...
import ug.go.agriculture.MAAIF_Extension.daes.profiling.otherplayers.NetworkStateChecker;
import ug.go.agriculture.MAAIF_Extension.daes.profiling.otherplayers.OtherPlayer;
import ug.go.agriculture.MAAIF_Extension.daes.profiling.otherplayers.OtherplayerAdapter;
import ug.go.agriculture.MAAIF_Extension.daes.profiling.otherplayers.Main;
...
public class Review extends Activity {
    ...
    private TextView other_market_players_count;
    private TextView no_other_market_players_text;
    /*
     * this is the url to our webservice
     * make sure you are using the ip instead of localhost
     * it will not work if you are using localhost
     * */
    public static final String URL_SAVE_NAME = "https://extension.agriculture.go.ug/api/saveMarketPlayer.php";
    public static final String URL_SYNC_NEW_EOIs = "https://extension.agriculture.go.ug/api/saveMarketPlayer.php";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.other_market_players_sync);
        ...
        other_market_players_count = (TextView) findViewById(R.id.other_market_players_count);
        no_other_market_players_text = (TextView) findViewById(R.id.no_other_market_players_text);
        listViewNames = (ListView) findViewById(R.id.listViewNames);

        //calling the method to load all the stored other market players
        loadOtherMarketPlayers();

        //the broadcast receiver to update sync status
        broadcastReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                //loading other market players
                loadOtherMarketPlayers();
            }
        };
     ...
     * */
    private void loadOtherMarketPlayers() {
        otherPlayers.clear();
        Cursor cursor = db.getOtherMarketPlayers();
        int count = cursor.getCount();
        other_market_players_count.setText("Other market players count: " + String.valueOf(count));

        if (count == 0) {
            no_other_market_players_text.setText("No other market players registered yet...");
        } else {
            no_other_market_players_text.setVisibility(View.GONE);
            if (cursor.moveToFirst()) {
                do {
                    @SuppressLint("Range") OtherPlayer otherPlayer = new OtherPlayer(
            ...
            otherplayerAdapter = new OtherplayerAdapter(this, R.layout.names, otherPlayers);
            listViewNames.setAdapter(otherplayerAdapter);
        }
    }
    ...
    private void saveNameToServer() {
        ...
        progressDialog.setMessage("Saving other market player...");
        progressDialog.show();
